...
def median(x):
	return x[population(x)//2] if population(x)%2==1 else (x[population(x)//2-1] + x[population(x)//2])/2
...
def class_mean(x):
	return sum([class_midpoint(x, i) for i in range(len(x))])/sum([class_frequency(x, i) for i in range(len(x))])
